...
int main(){
    ...
    // 噪音处理,使用高斯去除噪声,ksize值一般选择3
    cv::GaussianBlur(OriginImage,RemovalNoiseImage ,cv::Size(7,7),0);
    cv::imshow("RemovalNoiseImage",RemovalNoiseImage);
    if(cv::waitKey(0)=='q'){
        cv::imwrite("/Users/liquanyan/Desktop/OpenCV_CPP/image/gaussianBlur/ksize7.jpg", RemovalNoiseImage);
